...
/**
 * Created by johnsmac on 4/20/16.
 */
...
//    }
     ...
     * @return 身份证无误返回传入的身份证号
     */
    public static Boolean validate_effective(String idcardNumber, NoteIDError note) {
        ...
        if (Ai.length() == 15 | Ai.length() == 18) {
            ...
            note.showIdErroArea(LENGTH_ERROR);
            return false;
        }
        ...
        for (int i = 0; i < NEW_CARD_NUMBER_LENGTH - 1; i++) {
            char ch = Ai.charAt(i);
            if (ch < '0' || ch > '9') {
                note.showIdErroArea(NUMBER_ERROR);
                return false;
            }
        }
        ...
        try {
            ...
            if (null == birthDate) {
                note.showIdErroArea(DATE_ERROR);
                return false;
            }
